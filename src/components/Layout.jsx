  ...
  return (
    <div className="min-h-screen bg-secondary-50">
        ...
        <div className="flex h-full flex-col bg-white shadow-strong">
          ...
          <div className="flex h-16 items-center justify-between px-6 border-b border-secondary-200">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-xl font-bold text-secondary-900">Anvi CRM</h1>
            </div>
            <button 
              onClick={() => setSidebarOpen(false)}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-secondary-500 truncate">{item.description}</div>
                  </div>
                </Link>
              );
            })}
          ...
          <div className="border-t border-secondary-200 p-4">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-medium text-secondary-900 truncate">{user?.name}</div>
                <div className="text-sm text-secondary-500 truncate">{user?.email}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="border-t border-secondary-200 p-4">
            <div className="relative">
              <button
                ...
                className="w-full flex items-center space-x-3 p-3 rounded-xl hover:bg-secondary-50 transition-colors"
              >
                <div className="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center">
                  <span className="text-primary-700 font-medium text-sm">
                    {user?.name?.charAt(0)?.toUpperCase()}
                ...
                <div className="flex-1 min-w-0 text-left">
                  <div className="font-medium text-secondary-900 truncate">{user?.name}</div>
                  <div className="text-sm text-secondary-500 truncate">{user?.role}</div>
                </div>
              ...
              {/* User dropdown */}
              {userMenuOpen && (
                <div className="absolute bottom-full left-0 right-0 mb-2 bg-white rounded-xl shadow-strong border border-secondary-200 py-2">
                  <button
                    onClick={() => {
      ...
      <div className="lg:pl-72">
        {/* Top bar */}
        <div className="sticky top-0 z-30 flex h-16 bg-white/80 backdrop-blur-xl border-b border-secondary-200/50 shadow-sm">
          {/* Mobile menu button */}
          <button
            className="px-4 text-secondary-500 hover:text-secondary-700 focus:outline-none lg:hidden"
          ...
          <div className="flex-1 flex items-center px-4 lg:px-6">
            <div className="relative max-w-md w-full">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 text-sm bg-secondary-50 border border-secondary-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all"
              />
            </div>
          </div>
