  ...
  return `
      ...
      <body>
        <div class="invoice">
          ...
          <div class="customer" style="display:grid;grid-template-columns:1fr 1fr;gap:8px;align-items:start">
            ${invoice.customer?.name ? `<div style="font-weight:700">${invoice.customer.name}</div>` : '<div></div>'}
            ${invoice.customer?.phone ? `<div>Mobile: ${invoice.customer.phone}</div>` : '<div></div>'}
            ${invoice.destination ? `<div style="grid-column:1/-1">Destination: ${invoice.destination}</div>` : ''}
            ${invoice.buyerDetails?.gstin ? `<div style="grid-column:1/-1">GSTIN/UIN: ${invoice.buyerDetails.gstin}</div>` : ''}
          </div>
