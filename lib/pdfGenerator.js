...
export const generateInvoicePDF = async (invoice, template = 'compact') => {
  let browser = null;
  ...
  try {
    ...
    const html = template === 'detailed' ? generateProfessionalTemplate(invoice) : generateCompactTemplate(invoice);
...
function generateProfessionalTemplate(invoice) {
  return `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div class="invoice">
          ...
          <div class="customer" style="display:flex;gap:12px">
            <div style="flex:1">
              ${invoice.customer?.name ? `<div style="font-weight:700;margin-bottom:4px">${invoice.customer.name}</div>` : ''}
              ...
              ${invoice.destination ? `<div style="margin-top:6px">Destination: ${invoice.destination}</div>` : ''}
            </div>
              ...
              ${invoice.items.map((item, index) => {
                ...
                return `
                  <tr>
                    ...
                    <td class="right">${itemGross.toFixed(2)}</td>
                    <td class="right">${discountAmount > 0 ? `${discountAmount.toFixed(2)}` : '-'}</td>
          ...
          <div class="amount-words">
            ...
            Total Amount (Before Discount): ₹${(invoice.subtotal || 0).toFixed(2)}<br>
            ${(invoice.discount > 0 || (invoice.items && invoice.items.some(item => item.discount > 0))) ? `Total Discount Applied: -₹${((invoice.discount || 0) + (invoice.items?.reduce((sum, item) => {
              const itemDiscount = parseFloat(item.discount) || 0;
              const itemGross = item.quantity * item.price;
              return sum + (item.discountType === 'percentage' ? (itemGross * itemDiscount) / 100 : itemDiscount);
            }, 0) || 0)).toFixed(2)}<br>` : ''}
            Taxable Amount: ₹${(invoice.totalTaxableAmount || invoice.subtotal).toFixed(2)}<br>
            Total GST (18%): ₹${((invoice.totalCgst || 0) + (invoice.totalSgst || 0)).toFixed(2)}<br>
            <strong>Grand Total: ₹${(invoice.grandTotal || invoice.total).toFixed(2)}</strong><br>
            ${(invoice.paidAmount > 0) ? `<strong>Amount Paid: ₹${invoice.paidAmount.toFixed(2)}</strong><br>` : ''}
            ${(invoice.pendingAmount > 0) ? `<strong>Pending Amount: ₹${invoice.pendingAmount.toFixed(2)}</strong>` : ''}
          </div>

          <div class="amount-words">
            <strong>Amount in Words:</strong> INR ${invoice.amountInWords || 'Zero'} Only
          </div>
...
function generateCompactTemplate(invoice) {
  return `
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .company-info { font-size: 12px; line-height: 1.6; color: #666; }
        .invoice-title { text-align: center; background: #333; color: white; padding: 10px; font-size: 20px; font-weight: bold; }
        .details { display: flex; padding: 20px; border-bottom: 2px solid #333; }
        .customer { flex: 1; }
        ...
        .total-row { background: #f8f9fa; font-weight: bold; font-size: 14px; }
        .footer { padding: 20px; border-top: 2px solid #333; }
        .amount-words { margin: 20px; padding: 15px; background: #f8f9fa; border-left: 4px solid #333; }
      </style>
    </head>
    <body>
      <div class="invoice">
        <div class="header">
          <div class="company-name">${invoice.companyDetails?.name || 'Company Name'}</div>
          <div class="company-info">
            ${invoice.companyDetails?.address || ''}<br>
            ${invoice.companyDetails?.mobile ? `Phone: ${invoice.companyDetails.mobile}` : ''}
        ...
        <div class="details">
          ...
          <div class="invoice-info">
            ...
            <div><strong>Payment:</strong> ${invoice.paymentMethod?.toUpperCase() || 'CASH'}</div>
          </div>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width: 80px" class="text-right">Gross</th>
          ...
          <tbody>
            ${invoice.items.map((item, index) => {
              ...
              <tr>
                ...
                <td class="text-right">₹${itemGross.toFixed(2)}</td>
                <td class="text-right">${discountAmount > 0 ? `₹${discountAmount.toFixed(2)}` : '-'}</td>
                <td class="text-right">₹${item.total.toFixed(2)}</td>
              </tr>
            `}).join('')}
            <tr class="total-row">
              <td colspan="6" class="text-right">GRAND TOTAL</td>
              <td class="text-right">₹${(invoice.grandTotal || invoice.total).toFixed(2)}</td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="amount-words">
          <strong>Amount in Words:</strong> ${invoice.amountInWords || 'Zero'} Only
        </div>
        
        ${(invoice.discount > 0 || (invoice.items && invoice.items.some(item => item.discount > 0))) ? `
        <div class="amount-words">
          <strong>Total Discount Applied:</strong> ₹${((invoice.discount || 0) + (invoice.items?.reduce((sum, item) => {
            const itemDiscount = parseFloat(item.discount) || 0;
            const itemGross = item.quantity * item.price;
            return sum + (item.discountType === 'percentage' ? (itemGross * itemDiscount) / 100 : itemDiscount);
          }, 0) || 0)).toFixed(2)}
        ...
        <div class="footer">
          <div style="text-align: center; margin-top: 40px;">
            <div style="border-top: 1px solid #333; display: inline-block; padding-top: 10px; min-width: 200px;">
