...
export const tenantIsolation = async (req, res, next) => {
  try {
    ...
    // If org not found, allow request to proceed (will return empty data)
    if (!org) {
      return next();
    }
    ...
    if (org.subscription?.isBlocked || org.subscription?.status === 'blocked') {
      return res.status(403).json({ 
        success: false, 
        blocked: true,
        message: org.subscription.blockReason || 'Your CRM access has been suspended.',
...
export const checkSubscriptionLimit = (limitType) => {
  return async (req, res, next) => {
    try {
      if (req.userRole === 'superadmin' || !req.organization) return next();
